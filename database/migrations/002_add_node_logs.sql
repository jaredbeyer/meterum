-- Migration to add node logging functionality
-- This creates a table to store logs from nodes for debugging and monitoring
...
CREATE INDEX idx_node_logs_node_uuid ON node_logs(node_uuid);
CREATE INDEX idx_node_logs_timestamp ON node_logs(timestamp DESC);
CREATE INDEX idx_node_logs_level ON node_logs(log_level);
...
-- Optional: Create a scheduled job to clean up old logs (requires pg_cron extension)
-- CREATE EXTENSION IF NOT EXISTS pg_cron;
-- SELECT cron.schedule('cleanup-node-logs', '0 2 * * *', 'SELECT cleanup_old_node_logs();');